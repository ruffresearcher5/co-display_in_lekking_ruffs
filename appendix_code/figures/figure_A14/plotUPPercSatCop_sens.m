function SatPerPlot = plotUPPercSatCop_sens( Sat_Com_ab )
%%%%%Subplot function for supplementary figure A14_b_weak-strong%%%%%
    ...
    case "strong"
        ...
        text(-0.96,6.75,0,'\eta','color',[0.0 0.0 0.0],'FontSize',21)
            ...
            'Normal')   
                ...
                set(SatPerPlot,'Position',[0.32 0.7093 0.1566 0.2157])
                caxis manual
                caxis([0 20]);
                grid off
                
     case "medium"
        SatPerPlot=subplot(3,4,6);
                Cbar3(MatPercCopSat,MatPercCopSat)
                xlim(X_axesLimits)
                ylim(Y_axesLimits)
                xticks(XandY_ticks)
        ...
        text(-0.96,6.75,0,'\eta','color',[0.0 0.0 0.0],'FontSize',21)
                ...
                a = get(SatPerPlot,'XTickLabel'); 
                set(SatPerPlot,'XTickLabel',a,'Fontsize',15) 
                set(SatPerPlot,'Position',[0.32 0.4096 0.1566 0.2157])
                colormap(colormap_sat_percent( ));
                caxis manual
                caxis([0 20]);
                grid off
      case "weak"
        ...
        text(-0.96,6.75,0,'\eta','color',[0.0 0.0 0.0],'FontSize',21)
                ...
                a = get(SatPerPlot,'XTickLabel'); 
                set(SatPerPlot,'XTickLabel',a,'Fontsize',15)
                set(SatPerPlot,'Position',[0.32 0.1100 0.1566 0.2157])
                colormap(colormap_sat_percent( ));
                caxis manual
                caxis([0 20]);
                grid off
