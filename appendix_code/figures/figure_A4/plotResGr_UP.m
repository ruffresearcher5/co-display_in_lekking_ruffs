...
%%%%%Subplot function for supplementary figure B4_B%%%%%
...
GrupPlot=subplot(2,2,2);
        ...
        el = 90;
        view(az, el);
        a = get(GrupPlot,'XTickLabel'); 
        set(GrupPlot,'XTickLabel',a,'Fontsize',18)
        title(Title_plot,'FontSize',24)
        title (Title_plot, 'position' , Title_position)   
        colormap(flipud(hot));
        set(GrupPlot,'Position',[0.399 0.6 0.3347 0.3412])
        set(GrupPlot,'TickDir','out')
        caxis manual
        caxis([-2.00 4.00]);
        grid off
