...
FrskPlot=subplot(2,2,3);
        ...
        caxis manual
        caxis([0.00 8.00]);
        a = get(FrskPlot,'XTickLabel'); 
        set(FrskPlot,'XTickLabel',a,'Fontsize',18)
        title(Title_plot,'FontSize',24)
        title (Title_plot, 'position' , Title_position)   
        colormap(jet);
        set(FrskPlot,'zticklabel',num2str(get(gca,'ztick')','%.2f'),...
            'TickDir','out')
        set(FrskPlot,'Position',[0.174 0.15 0.3347 0.3412])
        grid off
        set(FrskPlot,'TickDir','out')
