...
FrnPlot=subplot(2,2,1);
        ...
        caxis manual
        caxis([0.00 8.00]);
        a = get(FrnPlot,'XTickLabel'); 
        set(FrnPlot,'XTickLabel',a,'Fontsize',18)
        title(Title_plot,'FontSize',24)
        title (Title_plot, 'position' , Title_position)   
        colormap(jet);
        set(FrnPlot,'zticklabel',num2str(get(gca,'ztick')','%.2f'),...
            'TickDir','out')
        set(FrnPlot,'Position',[0.174 0.6 0.3347 0.3412])
        grid off
        set(FrnPlot,'TickDir','out')
