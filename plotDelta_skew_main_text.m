function SkewPlot = plotDelta_skew_main_text()
%%%%%Subplot function for supplementary figure 1_D%%%%%
%Plot of predicted change in resident mating skew given 
...
%L=1
MatSkew(1:7,1)=[Co_display_delta_Skew(1,1,H,Sat_Com_ab,Comp_level),...
    nan,nan,nan,nan,nan,nan];
%L=2
MatSkew(1:7,2)=[Co_display_delta_Skew(2,1,H,Sat_Com_ab,Comp_level),...
    Co_display_delta_Skew(2,2,H,Sat_Com_ab,Comp_level),nan,nan,nan,nan,nan];
%L=3
MatSkew(1:7,3)=[Co_display_delta_Skew(3,1,H,Sat_Com_ab,Comp_level),...
    Co_display_delta_Skew(3,2,H,Sat_Com_ab,Comp_level),...
    Co_display_delta_Skew(3,3,H,Sat_Com_ab,Comp_level),...
    nan,nan,nan,nan];
%L=4
MatSkew(1:7,4)=[Co_display_delta_Skew(4,1,H,Sat_Com_ab,Comp_level),...
...
%L=5
MatSkew(1:7,5)=[Co_display_delta_Skew(5,1,H,Sat_Com_ab,Comp_level),...
...
%L=6
MatSkew(1:7,6)=[Co_display_delta_Skew(6,1,H,Sat_Com_ab,Comp_level),...
...
%L=7
MatSkew(1:7,7)=[Co_display_delta_Skew(7,1,H,Sat_Com_ab,Comp_level),...
...
%Make a figure
        SkewPlot=subplot(2,2,4);
                Cbar3(MatSkew,MatSkew)
                ...
                yticks(XandY_ticks)
                yticklabels(["" "" "" ""])
                text(-0.7,0.87,0,'\alpha','Color','black','FontSize',18)
                text(-0.7,2.87,0,'\gamma','Color','black','FontSize',18)
                text(-0.7,4.87,0,'\epsilon','Color','black','FontSize',18)
                text(-0.7,6.87,0,'\eta','Color','black','FontSize',18)
                az = 0;
                el = 90;
                view(az, el);
                a = get(SkewPlot,'XTickLabel'); 
                set(SkewPlot,'XTickLabel',a,'Fontsize',18)
                Title_plot="D";
                ...
                caxis manual
                caxis([-0.81 0.00]);
                set(SkewPlot,'Position',[0.47 0.1100 0.3347 0.3412])
                C_Bticks=[-0.81 -0.40 0.00];
                hr1 = colorbar('Ticks',[],'Position',...
                    [0.728 0.11 0.02 0.345],...
                    'YAxisLocation','right','XTick', C_Bticks);
                    hr1.Limits = [-0.81 0.00];
                    text(8.5,6.6,0,...
                        {'Mating skew change'},...
